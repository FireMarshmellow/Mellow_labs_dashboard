...
- Files are stored on disk under the directory specified by `UPLOADS_DIR` (defaults to `./uploads`).
...
- For remote exposure or HTTPS, place the app behind a reverse proxy (nginx/Caddy/Traefik) or tunnel via SSH.
